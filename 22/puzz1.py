...
class Stack():
    ...
    def deal_with_increment(self, increment):
        ...
        new_stack.deck = ['.'] * len(self.deck)
        for i in range(len(self.deck)):
            new_stack.deck[(i * increment) % len(new_stack.deck)] = self.deck[i]
        self.deck = new_stack.deck

    def __repr__(self):
        return ' '.join([f'{i:2g}' if type(i)==int else i for i in self.deck ])
...
def process_instructions(fname, size):
    ...
    print('0:', s)
    i = 1
    for tup in tqdm(instr):
        if tup[:2] == ('deal', 'into'):
            s.deal_into_new_stack()
        elif tup[:2] == ('deal', 'with'):
            s.deal_with_increment(int(tup[-1]))
        elif tup[0] == 'cut':
            s.cut(int(tup[-1]))
        else:
...
if __name__ == '__main__':
    ...
    process_instructions('22/test_input3', 10)
    # assert str(process_instructions('22/test_input1', 10)) == '0 3 6 9 2 5 8 1 4 7'
    # assert str(process_instructions('22/test_input2', 10)) == '3 0 7 4 1 8 5 2 9 6'
    # assert str(process_instructions('22/test_input3', 10)) == '6 3 0 7 4 1 8 5 2 9'
    # assert str(process_instructions('22/test_input4', 10)) == ' 9  2  5  8  1  4  7  0  3  6'

    # s = process_instructions('22/input', 10007)
    # print(s.deck.index(2019))

    # Answer is 3143

    # size = 10
    # print('First operation stack')
    # s = Stack(size)
    # print(f'0: {s}')
    # s.deal_with_increment(3)
    # print(f'1: {s}')
    # s.cut(3)
    # print(f'2: {s}')
    # print('')
