...
def run_intcode(inp):
    ...
    while True:
        ...
        inp1_idx = inp[pos+1]
        inp2_idx = inp[pos+2]
        output_idx = inp[pos+3]

        if opcode == 1:
            inp[output_idx] = inp[inp1_idx] + inp[inp2_idx]
        elif opcode == 2:
            inp[output_idx] = inp[inp1_idx] * inp[inp2_idx]
...
if __name__ == '__main__':
    ...
    with open('02/input', 'r') as f:
        orig_inp = f.readline()
    orig_inp = [int(i) for i in orig_inp.split(',')]
    
    for i, j in itertools.product(range(100), range(100)):
        # replace position 1 with the value i and replace position 2 with the
        # value j.
        inp = orig_inp.copy()
        inp[1] = i
        inp[2] = j
